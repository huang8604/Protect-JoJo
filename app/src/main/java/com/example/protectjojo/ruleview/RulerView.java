...
public class RulerView extends View implements ScrollChange {
    ...
    private int dpForTextEnd;
    private int dpForLineEnd;
    ...
    private void initDistanceForDp() {
        mLineWidth = dp2px(2);
        mDoubleWidth = dp2px(4);
        mLineSpace = dp2px(12);
        mSmallLineHeight = dp2px(70);
        mLongLineHeight = dp2px(100);
        dpForTextEnd = dp2px(1);
        dpForLineEnd = dp2px(25);
    }
     ...
     * @param end
     */
    public void setScope(int start, int end,float offSet) {
    ...
    private void drawRuler(Canvas canvas) {
        ...
        for (int index = 0; index <= counts; index++) {
            ...
            if(longLine){
                mRect.left = index * mLineSpace + lineCount + mMarginLeft - mDoubleWidth/2;
                mRect.right = mRect.left + mLineWidth + mDoubleWidth/2;
            }else{
    ...
    private int getStartY(boolean isLong) {
        if (isLong) {
            return getMeasuredHeight() - mLongLineHeight - dpForLineEnd;
        } else {
            return getMeasuredHeight() - mLongLineHeight/2 - mSmallLineHeight/2 - dpForLineEnd;
        }
    }
     ...
     * @return
     */
    private int getEndY(boolean isLong) {
        if (isLong) {
            return getMeasuredHeight() - dpForLineEnd;
        } else {
            return getMeasuredHeight() - (mLongLineHeight - mSmallLineHeight)/2 - dpForLineEnd;
        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (Math.abs(xVelocity) >= mMinVelocity) {
                    ...
                    int velocityX = (int) (xVelocity * 0.25);
